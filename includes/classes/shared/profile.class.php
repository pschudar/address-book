...
trait Profile {
    ...
    private function buildReadLink(bool $read, string $module, string $util) {
        switch ($read) {
            case true:
                $link = '<a class="text-primary" href="' . $util::urlFor('/staff/' . $module . '/view.php?id=') . $util::hu($this->id) . '">View</a>';
                return $link;
        } return;
    }
    ...
    private function buildUpdateLink(bool $update, string $pipe, string $module, string $util) {
        switch ($update) {
            case true:
                $link = ' ' . $pipe . '  <a class="text-muted" href="' . $util::urlFor('/staff/' . $module . '/edit.php?id=') . $util::hu($this->id) . '">Edit</a>';
                return $link;
        } return;
    }
    ...
    private function buildDeleteLink(bool $delete, string $pipe, string $module, string $util) {
        ...
        switch ($delete) {
            case true:
                $link = ' ' . $pipe . ' <a data-id="' . $util::hu($this->id) . '" data-name="' . $util::h($this->fullName()) . '" data-toggle="modal" data-target="' . $target . '" class="triggerDelete text-danger" href="' . $util::urlFor('/staff/' . $module . '/delete.php?id=') . $util::hu($this->id) . '">Delete</a>';
                return $link;
        } return;
    }
