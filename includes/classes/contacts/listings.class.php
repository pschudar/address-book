...
class Listings extends \database\DatabaseObject {
    ...
    public function printPhoneNumbers() {
        ...
        if (\utility\Utility::hasPresence($this->phone_home)) {
            $phone .= 'Home: <a class="contact" href="tel:+1' . $this->phone_home . '">' . $this->phone_home . '</a><br>';
        }
        if (\utility\Utility::hasPresence($this->phone_mobile)) {
            $phone .= 'Mobile: <a class="contact" href="tel:+1' . $this->phone_mobile . '">' . $this->phone_mobile . '</a><br>';
        }
        return $phone;
    }
    ...
    public function formatAge($dob, $index = true) {
        $date = \utility\Utility::h($dob);
        $return = null;
        if($index) {
            $return = '--';
        }
        if(\utility\Utility::isBlank($date)) {
            return $return;
        }
    ...
    public function dateOfBirth() {
        if(\utility\Utility::isBlank($this->date_of_birth)) {
            return '--';
        }
        ...
        return \utility\Utility::formatDate($dateOfBirth, 'Y-m-d');
    }
