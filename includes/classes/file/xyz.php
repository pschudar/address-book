...
/**
 * UserImageXref creates, reads, updates, and deletes Image Cross References.
 * 
 * Uses the infrastructure from DatabaseObject class to perform CRUD actions
 */
class UserImageXref extends \database\DatabaseObject {

    static protected $table_name = 'ab_user_image_xref';
    static protected $db_columns = ['user_id', 'image_id'];
    public $user_id;
    public $image_id;
    public $filename;
    ...
    public static function removeProfileImage(int $contactId) {
        $current = UserImageXref::getCurrentImage($contactId);
        ...
        switch ($current) {
            ...
            default:
                ...
                $img = UserImageXref::removeImageRecords($current->user_id, $current->image_id, PROFILE_IMAGE_PATH, $current->filename);
                if ($img):
                    return true;
                else:
                    return false;
    ...
    private static function removeImageRecords(int $contactId, int $imageId, string $imgPath, string $filename) {
        $sql = 'DELETE cix, img FROM ab_user_image_xref cix, ab_images img WHERE cix.user_id = ' . $contactId . ' AND img.id = ' . $imageId;
    ...
    private static function getCurrentImage(int $contactId) {
        $sql = 'SELECT cix.user_id, cix.image_id, i.filename FROM ab_user_image_xref cix LEFT JOIN ab_images i on cix.image_id = i.id WHERE cix.user_id = ' . $contactId . ' LIMIT 1';
